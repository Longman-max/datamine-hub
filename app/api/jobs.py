...
async def spawn_job(payload: JobSpawnRequest, db: aiosqlite.Connection = Depends(get_db)):
    ...
    try:
        ...
        process = subprocess.Popen(
            [sys.executable, "agent_runner.py", "--role", payload.role, "--url", payload.target_url],
            env=env,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )
        
        print(f"[DISPATCHER] Spawned {payload.role} agent (PID: {process.pid}) for target: {payload.target_url}")
